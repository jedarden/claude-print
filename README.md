...
- **`HOME` must name an existing, writable directory** for the user running `claude-print` — a non-empty value alone is not enough. Claude Code configuration, trust state, and transcripts are resolved beneath this directory, and every entry point except `--help` (prompt runs, `--check`, `--version`, `serve`) validates it at startup: the path must exist, be a directory, and pass a create-write-remove probe of a temporary file inside it. A `HOME` that is unset, empty, missing, or read-only is a setup error, never a `/root` fallback: the process exits with status 2 and an actionable message — `HOME environment variable not set or empty; set HOME to the user's home directory`, or a path-specific form such as `HOME path '/home/service' is not writable: ...; grant write permission or set HOME to an existing, writable directory` (`json` and `stream-json` output carries the same message in `error_message`). Setting `XDG_CONFIG_HOME` is the one partial exception: it relocates the config file so that resolution no longer consults `HOME`, but it does not remove the requirement — Claude Code state and transcripts still live under `$HOME`. See [HOME in containers and chroots](#home-in-containers-and-chroots) for provisioning recipes.
...
Every downloaded artifact is verified against the release's published `sha256sums.txt` before it is installed or executed. A missing manifest, an asset with no checksum entry, or any digest mismatch aborts the install with nothing placed — the check fails closed. Both causes of a digest mismatch are pinned separately: a tampered checksum entry (the manifest's digest altered, the artifact itself pristine) and a partially transferred artifact (only a prefix of the bytes the manifest digests) abort identically. The `mock_claude` fixture remains optional: a release whose manifest does not list it skips the fixture instead of failing. `tests/install_sh.rs` pins each of these guarantees adversarially: it serves a forged release through `CLAUDE_PRINT_RELEASE_URL` and asserts that every abort leaves nothing installed.
