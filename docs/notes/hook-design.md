...
Claude Code merges this with any user hooks from `~/.claude/settings.json`, so both user hooks and the relay hooks fire. **Measured on claude 2.1.270 (2026-09-13; re-confirmed unchanged through 2.1.282 on 2026-09-24 — `docs/notes/claude-contract-probes.md`):** merge confirmed for user- and project-source hooks alongside the relay hook — every loaded source fires on each hook event. Cross-source firing *order* is not contractual: standard-source hooks typically start first (1–3 ms ahead), but concurrent firing — and runs where the relay started before the standard-source hook — were observed, and the dedicated sleeping-hook probe re-measured on 2.1.282 (2026-09-25, `scripts/probe-stop-edge-contracts.sh` Arm S: concurrent execution in 12/12 event pairs, relay started first in 6/12) confirms concurrency is the norm, not an outlier. Nothing in claude-print may depend on the relay firing last; the payload may arrive while user Stop hooks are still running, which is safe because the two consumers are independent. The merge semantics apply per event — the UserPromptSubmit relay merges with any user UserPromptSubmit hooks exactly as the Stop relay does (PO-1).
...
- `--setting-sources=` (empty) is forwarded to claude — suppresses loading of standard settings sources (**measured** on claude 2.1.270, re-confirmed unchanged through 2.1.282: the empty spelling is accepted, suppresses every standard source, and does not suppress the `--settings` file; the alternative spelling `=none` is rejected outright — exit 1 before session start)
