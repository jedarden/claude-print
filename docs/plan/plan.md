...
│   ├── probe-claude-contracts.sh          # live PO-1/PO-2/OQ-1/OQ-2 probes (claude 2.1.283; docs/notes/claude-contract-probes.md)
...
| `--check` | Run installation self-test: verify openpty, mkfifo, optional PTY round-trip with mock_claude, and the billing env-input force (`CLAUDE_CODE_ENTRYPOINT=cli` in the constructed child env). Exits 0 on all checks passed, 2 on any failure. |
...
**Hook merge ordering:** The relay hook's `"timeout": 10` applies only to the relay hook itself — it does not affect the user's hooks. **Measured (OQ-1 resolved, claude 2.1.270):** cross-source firing order is **not contractual**. In the four measured Stop/SessionStart pairs the standard-source hook started 2–3 ms ahead of the relay hook, but a dedicated start/end-timestamp probe (project hook sleeping 300 ms, relay 0 ms) observed the relay **starting before** the project hook in 1 of 4 runs, with the two hooks running concurrently. **The dedicated probe was re-measured on the pinned 2.1.282 (2026-09-25, `scripts/probe-stop-edge-contracts.sh` Arm S): concurrent execution in 12/12 event pairs, relay-start-first in 6/12 — and on 2.1.283 (2026-09-26): concurrent 12/12, relay-start-first 4/12** — same shape, later versions. The guaranteed property is only that every loaded source fires (merge). Consequence: claude-print **can** observe the Stop payload while user Stop hooks (e.g. a JSONL post-processor) are still running. That window is accepted, not ordered away — the two consumers are independent (claude-print reads payload + transcript and tears down; user hooks post-process separately), no later turn depends on user-hook output in the one-prompt session shape, and the existing teardown bounds (`/exit` window, then SIGTERM/SIGKILL; `--stop-hook-timeout` watchdog) are unchanged. Nothing in claude-print may depend on the relay firing last. See `docs/notes/claude-contract-probes.md`.
...
|---|-----------|-----------|
| EC-1 | Two `claude-print` instances on the same `cwd` concurrently | Each has its own `session_id` and JSONL file. FIFO paths are per-pid — no cross-contamination. Transcripts are per-session by construction too (claudepr-a927ec0c, fixed 2026-09): the stream-json reader binds to the exact transcript the per-drive UserPromptSubmit identity relay writes (`session-identity.json`, at prompt-submission time — before any assistant event exists), so concurrent same-cwd invocations never forward a sibling's events. This holds for every supported configuration: real `claude` on the stateless and pooled paths, and `mock-claude` (which emulates the relay hook). Residual limit — identity-less sessions only (`claude` without UserPromptSubmit hook support): the reader then binds only a single unambiguous new transcript (the sole `.jsonl` created after the injection snapshot, held for a 250 ms grace) and otherwise forwards nothing live until the Stop payload retargets it from the snapshot offset at drain — output arrives whole and uncontaminated, but not streamed, and a near-simultaneous same-cwd sibling can still mis-attribute the live prefix. `text`/`json` are exact in all configurations (transcript resolved by the Stop payload's `session_id`). Pinned by `tests/pool_adversarial_e2e.rs::concurrent_clients_each_drive_a_distinct_worker`, the binding-ladder unit tests in `src/emitter.rs`, and the identity-less fallback in `tests/integration/scenarios.rs`. |
...
5. Verify the billing env-input contract: rebuild the child environment over one that inherited `CLAUDE_CODE_ENTRYPOINT=sdk-cli` plus every scrubbed session marker, and confirm the shipped binary still produces exactly one `CLAUDE_CODE_ENTRYPOINT=cli` and none of the markers. Credential-free; the JSON-evidence half of the contract (the transcript's `entrypoint` field) stays with `scripts/check-billing.sh` and the AS-4 canary.
6. Scan `$TMPDIR` for leftover `claude-print-*` directories older than 1 hour and report them as warnings (does not fail the check). Example message: `WARNING: found orphaned temp dir /tmp/claude-print-12345-abc (1.2h old) — run rm -rf to clean up`. Passing `--check --clean` removes the directories matched by this same scan and prints a `CLEANED` line for each one. `--clean` is rejected without `--check`, keeping the default self-test non-destructive and safe for scripts and CI.
7. Print `OK` or a specific failure message per step
...
| R-2 | `--settings` merge behavior changes in a Claude Code update; user hooks stop firing | Medium | Medium (user hooks silently broken) | PO-1 verified before Phase 2 and re-measured live on 2026-09-13 (claude 2.1.270 — merge confirmed, order not contractual) and re-confirmed unchanged on 2026-09-24 (claude 2.1.281 and 2.1.282) and on 2026-09-26 (claude 2.1.283, claudepr-72d0ba4c; `docs/notes/claude-contract-probes.md`); version-compat tests track `claude --version`; drift detection via `scripts/check-claude-version-bump.sh` (exit 1 = re-run due; procedure: `docs/notes/claude-contract-probes.md` §Maintenance); `cargo test --test claude_contracts -- --ignored` re-measures merge/suppression against the installed claude; wired as `scripts/contract-maintenance-gate.sh` (detect → re-run → evidence → follow-up), executed by the `claude-print-ci` workflow on every push as the FIRST quality gate — since claudepr-3094ab2e drift (or an indeterminate detection) fails the build: a version change requires the re-run + re-pin commit to land before CI goes green again, while the gate still files the follow-up issue and stamps the release-notes contract evidence (the probes themselves need model auth CI lacks, so they run host-side); `tests/contract_maintenance.rs` pins the wiring so it cannot silently detach from the plan and the maintenance doc |
...
|---|---------|--------|----------------------|
| OQ-1 | Does `--settings <file>` merge hooks with `~/.claude/settings.json` or replace them? | Phase 2 | **Resolved 2026-09-13 (claude 2.1.270).** Merges — project-, user- and `--settings`-source hooks all fired alongside each other in isolated probe runs. Firing order is **not contractual**: the typical pattern is standard-source-first (2–3 ms ahead), but a start/end-timestamp probe measured the relay starting before the project hook in 1 of 4 runs with concurrent execution (re-measured on the pinned 2.1.282, 2026-09-25 — concurrent 12/12 event pairs, relay-first 6/12 — and on 2.1.283, 2026-09-26: concurrent 12/12, relay-first 4/12). The read-race clause resolves as "cannot be ordered away": claude-print may observe the Stop while user Stop hooks are still running — acceptable because the consumers are independent and the one-prompt session shape means no later turn depends on user-hook output (teardown bounds unchanged). Nothing may depend on the relay firing last. PO-1 in-process fallback not needed. Evidence: `docs/notes/claude-contract-probes.md`; live re-verification: `cargo test --test claude_contracts -- --ignored` |
