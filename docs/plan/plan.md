...
| R-2 | `--settings` merge behavior changes in a Claude Code update; user hooks stop firing | Medium | Medium (user hooks silently broken) | PO-1 verified before Phase 2 and re-measured live on 2026-09-13 (claude 2.1.270 — merge confirmed, order not contractual; `docs/notes/claude-contract-probes.md`); version-compat tests track `claude --version`; drift detection via `scripts/check-claude-version-bump.sh` (exit 1 = re-run due; procedure: `docs/notes/claude-contract-probes.md` §Maintenance); `cargo test --test claude_contracts -- --ignored` re-measures merge/suppression against the installed claude; wired as `scripts/contract-maintenance-gate.sh` (detect → re-run → evidence → follow-up), executed by the `claude-print-ci` workflow on every push — drift alerts via follow-up issue and the release-notes contract-evidence stamp, not a red build (the probes need model auth CI lacks); `tests/contract_maintenance.rs` pins the wiring so it cannot silently detach from the plan and the maintenance doc |
