...
| `tests/contract_maintenance.rs` | Contract-probe maintenance wiring guard (beads claudepr-e8fc5744 + claudepr-3094ab2e): gate script + detector parse, the gate's exit-code/evidence/version-file/follow-up contract against a stubbed `claude` (hermetic, re-pin-proof), real-environment self-consistency (CI's exercise of the detection path), and template/docs wiring fragments — including the fatal drift wrapper that makes a version change fail CI until the re-pin lands (`scripts/contract-maintenance-gate.sh`, `claude-print-ci-workflowtemplate.yml`, §Maintenance, plan R-2) |
...
| `tests/stream_json_contract.rs` | Golden replay of the stream-json output contract (contract: `docs/notes/stream-json-contract.md`; fixtures version-pinned like the transcript captures — the `v2.1.270` in the filename is the pinned Claude version): a full replay of `stream_json_golden_v2.1.270.input.jsonl` (PTY-shaped, one instance of every byte-level case: compact and spaced JSON, unicode, a `thinking` block, split assistant records, a blank line, a CRLF line) through the real reader must be byte-identical to `stream_json_golden_v2.1.270.expected.jsonl`; identity binding with a snapshot offset forwards only post-injection lines without duplicating them; incrementally arriving lines replay in order; the pinned `transcript_v2.1.233` capture forwards verbatim including its `result` record; a final record without a trailing newline is still LF-terminated; and the two synthesized `result` error objects byte-match `stream_json_golden_v2.1.270.errors.jsonl`. The reader thread is driven in-process — no binary spawned |
