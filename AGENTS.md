...
| `scripts/probe-stop-edge-contracts.sh` | The edge measurements the 2.1.281/2.1.282 re-pins had left at their 2.1.270 originals, re-measured 2026-09-25 (claudepr-d9553d38) and owned by this script since: Arm S — sleeping-hook cross-source concurrency (project hook sleeps 300 ms, relay 0 ms; 2.1.282: concurrent 12/12 event pairs, relay-start-first 6/12, proving order is not contractual); Arm D — degraded-path Stop counts on permission-denied tool calls (2.1.282: 15/15 completed denied runs across three invocations fired exactly one Stop; the 2.1.270 extra-Stop hazard did not reproduce); Arm T — relay-hook timeout enforcement (added 2026-09-26, claudepr-352cf1df; hook sleeping 30 s past its 5 s per-hook `timeout`, 2.1.282: killed in 16/16 event firings across two invocations, every session exit 0 with the reply rendered, claude exiting 5.0 s after the Stop hook started in 8/8 runs). Part of the maintenance gate's `--run-probes` set | real claude + auth (15 model turns) |
