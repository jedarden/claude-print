...
| `tests/install_sh.rs` | `install.sh` end-to-end against a fake release directory (assets + `sha256sums.txt`) served via `CLAUDE_PRINT_RELEASE_URL=file://…`, temp `HOME`, fake `claude` on `PATH` so the post-install `--check` smoke passes. Two surfaces: release-artifact integrity verification (missing manifest, unlisted asset, and digest mismatch each abort with nothing placed) and the `claude-print.prev` rollback-copy semantics of `docs/notes/installer-rollback.md` — backup on upgrade (verbatim content, mode 755, no `mock_claude` copy), single-generation replacement across consecutive upgrades, no copy on a fresh install, and verify-before-backup ordering (a failed install disturbs neither the live binary nor an existing copy). Hermetic — no network, no real download (asset names pinned to x86_64, the only architecture CI publishes) |
