...
| `tests/probe_version_guard.rs` | Version-straddle guard for the contract probes (bead claudepr-9fe76ef4): `scripts/probe-version-guard.sh` under `bash` with a stubbed or flipping `claude` on a single-entry PATH of real-coreutils symlinks (the `contract_maintenance` pattern) — a stable binary resolves+ pins and stamps `verdict=single-version`, a mid-run version flip aborts exit 1 with the STRADDLED verdict, a repointed launcher cannot redirect the pinned run (single-version evidence + the `host-claude-moved-on` note), and a missing/unparsable claude fails closed exit 2; all four `probe-*.sh` are pinned to source the guard (begin+end, no private `CLAUDE_BIN` resolution); `scripts/contract-maintenance-gate.sh` brackets itself (a mid-gate flip → exit 2 INDETERMINATE with `version-stability: straddled` in contract-status.txt and the straddle next-steps); and the active `claude_contracts` fixture must carry a `version_guard` record whose `start`/`end` equal `claude_version` — or an honest `retroactive` provenance for the pre-guard 2.1.282 pin, allowed only for measurements older than the guard itself |
...
| `tests/stream_json_contract.rs` | Golden replay of the stream-json output contract (contract: `docs/notes/stream-json-contract.md`; fixtures version-pinned like the transcript captures — the `v2.1.283` in the filename is the pinned Claude version, re-measured live 2026-09-26 by claudepr-72d0ba4c with the `v2.1.270`/`v2.1.282` families retained as history): a full replay of `stream_json_golden_v2.1.283.input.jsonl` (PTY-shaped, one instance of every byte-level case: a 2.1.283 `mode` record — assistant records carry the 2.1.283 `serverClassifierRequest` field —, compact and spaced JSON, unicode, a `thinking` block, split assistant records sharing a `message.id`, a blank line, a CRLF line) through the real reader must be byte-identical to `stream_json_golden_v2.1.283.expected.jsonl`; identity binding with a snapshot offset forwards only post-injection lines without duplicating them; incrementally arriving lines replay in order; the pinned `transcript_v2.1.233` capture forwards verbatim including its `result` record; a final record without a trailing newline is still LF-terminated; and the two synthesized `result` error objects byte-match `stream_json_golden_v2.1.283.errors.jsonl`. The reader thread is driven in-process — no binary spawned |
...
| `config_entry_point_scope` | compiled-binaries | compiled CLI across `--help`/`--version`/`--check`/`serve` fast-fail plus one live daemon; mock-claude backend |
...
| `probe_version_guard` | repo-scripts | the guard library, the four probe wiring pins, the gate bracket, and the fixture/doc records under `bash` with stubbed and version-flipping `claude` stubs (the `contract_maintenance` pattern) |
...
| `platform_matrix_docs` | library-level | reads README, the CI WorkflowTemplate, install.sh, the installer suites' sources, `src/`, and Cargo.toml; spawns nothing |
...
| `scripts/probe-version-guard.sh` | The version-straddle guard every `probe-*.sh` sources (bead claudepr-9fe76ef4; `claude-contract-probes.md` §Version guard): `probe_version_guard_begin` resolves and PINS `CLAUDE_BIN` once (the auto-updater repoints the `~/.local/bin/claude` symlink mid-run — the 2026-09-24 straddle — so pinned invocations cannot be redirected; a pre-set `CLAUDE_BIN` pins a whole re-pin session), and `probe_version_guard_end`, the last line of every probe, re-runs `--version` and aborts the run as failed on any mismatch (exit 1, `verdict=STRADDLED` — the evidence mixes two versions and MUST NOT be pinned; exit 2 when the version cannot be determined), or stamps the machine-readable `verdict=single-version start=<v> end=<v> binary=<path>` line the re-pin records in the fixture's `version_guard` object. The maintenance gate brackets itself with the same start/end captures (`version-stability: straddled` → exit 2). Pinned by `tests/probe_version_guard.rs` | none — a sourced library, not a standalone probe |
...
| `scripts/probe-stop-edge-contracts.sh` | The edge measurements the 2.1.281/2.1.282 re-pins had left at their 2.1.270 originals, re-measured 2026-09-25 (claudepr-d9553d38) and owned by this script since: Arm S — sleeping-hook cross-source concurrency (project hook sleeps 300 ms, relay 0 ms; 2.1.282: concurrent 12/12 event pairs, relay-start-first 6/12; 2.1.283: concurrent 12/12, relay-start-first 4/12 — proving order is not contractual); Arm D — degraded-path Stop counts on permission-denied tool calls (2.1.282: 15/15 completed denied runs across three invocations fired exactly one Stop; 2.1.283: 5/5; the 2.1.270 extra-Stop hazard did not reproduce); Arm T — relay-hook timeout enforcement (added 2026-09-26, claudepr-352cf1df; hook sleeping 30 s past its 5 s per-hook `timeout`, 2.1.282: killed in 16/16 event firings across two invocations, every session exit 0 with the reply rendered, claude exiting 5.0 s after the Stop hook started in 8/8 runs). Part of the maintenance gate's `--run-probes` set | real claude + auth (15 model turns) |
