...
| `tests/contract_maintenance.rs` | Contract-probe maintenance wiring guard (bead claudepr-e8fc5744): gate script + detector parse, the gate's exit-code/evidence/version-file/follow-up contract against a stubbed `claude` (hermetic, re-pin-proof), real-environment self-consistency (CI's exercise of the detection path), and template/docs wiring fragments (`scripts/contract-maintenance-gate.sh`, `claude-print-ci-workflowtemplate.yml`, §Maintenance, plan R-2) |
